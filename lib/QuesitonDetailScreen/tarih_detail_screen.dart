...
class TarihDetailScreen extends StatefulWidget {
  const TarihDetailScreen({super.key});

  @override
  State<TarihDetailScreen> createState() => _TarihDetailScreenState();
}

class _TarihDetailScreenState extends State<TarihDetailScreen> {
  int _currentIndex = 0;
  List<Map<String, dynamic>> _tarihquestion = [];
  ...
  Future<void> _loadQuestions() async {
    QuerySnapshot snapshot = await FirebaseFirestore.instance
        .collection('tarihquestion')
        .get();

    setState(() {
      _tarihquestion = snapshot.docs
  ...
  void _nextQuestion() {
    if (_tarihquestion.isEmpty) return; // liste boşsa çık
    setState(() {
      if (_currentIndex < _tarihquestion.length - 1) {
        _currentIndex++;
      } else {
  ...
  Widget build(BuildContext context) {
    ...
    // Mevcut soru
    var currentQuestion = _tarihquestion[_currentIndex];
